...
function(r) {
    ...
    function($, fmt, w, utils, Temp, Page, datepicker) {
        ...
        function PageRender(options) {
            ...
            this.dataApi = {
                search: 'logPhone/findByPage',
                // 查询接口
                update: 'logPhone/update',
                // 修改状态
                save: 'logPhone/insert',
                // 修改状态
                enable: 'logPhone/enable',
                // 修改状态
                disable: 'logPhone/disable',
                // 修改状态
                del: 'logPhone/deleteByBatch',
                // 删除接口
                searchOne: 'logPhone/get'
            };
            // 默认查询参数
            this.searchOptions = {
                pageNum: 1,
            ...
            this.addModal.modal({
                ...
                onConfirm: function() {
                    if(that.eventType!='show'){
                         // 遍历数据
                        ['userPk','phonePk','salePrice','cost','ratio', 'dsc'].map(function(i) {
                            that.addOptions[i] = that.addModal.find('[name="' + i + '"]').val();
                        });
                        var userPk = that.addModal.find('[name="userPk"]').val();
                        var userName = that.addModal.find('[name="userPk"]').find('option[value="'+userPk+'"]').text();
                        if(userName.indexOf('-')>0){
                            that.addOptions['userName'] = userName.split('-')[0];
                            that.addOptions['phone'] = userName.split('-')[1];
                        }else{
                            utils.alert({
                                text: "请选择用户",
                                type: 'warning'
                        ...
                        var phonePk = that.addModal.find('[name="phonePk"]').val();
                        that.addOptions['name']  = that.addModal.find('[name="phonePk"]').find('option[value="'+phonePk+'"]').text();

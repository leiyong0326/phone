define([ '../require.config' ], function(r) {
	...
	require([ 'jquery','format', 'utils', 'template', 'pagination', 'datetimepicker', 'navbar', 'amazeui' ], function($,fmt, utils, Temp, Page, datepicker) {
		...
		function PageRender(options) {
			...
			this.dataApi = {
				search: 'scdTime/find', // 查询接口
				add: 'scdTime/saveOrUpdate', // 新增接口
				searchOne: 'scdTime//get', // 查询某条数据
				del: 'scdTime/delete', // 删除接口
			...
			this.addModal.modal({
				...
				onConfirm: function() {
					...
					// 遍历数据
					['name', 'startTime','endTime','num','typeName','typePk'].map(function(i) {
						that.addOptions[i] = that.addModal.find('[name="' + i + '"]').val();
						if(i == 'typeName') {
							that.addOptions[i] = that.addModal.find('[name="typeName"] option:selected').text();
						}
						if(i == 'typePk'){
						    that.addOptions[i] = that.addModal.find('[name="typeName"]').val();
						}
					});
		...
		PageRender.prototype = {
			...
			getType : function(callback) {
				...
				utils.ajax({
					...
					data.obj.map(function(i) {
						that.modalTemp.content.find('.type-name').append('<option value="' + i.id + '">' + i.name + '</option>');
					});
					callback();
				    }
				});
			...
			refreshModal: function() {
				...
				this.getType(function() {
					...
					that.modalTemp.content.find('.type-name option[value="' + that.addOptions.typePk + '"]').attr('selected', true);
				});
			...
			refreshData: function() {
				...
				this.upData(function(data) {
					...
					that.temp.render();
					that.pagination.options.pageTotal = data.total;
					that.pagination.refreshData(this.searchOptions.pageNum);
				});
			},

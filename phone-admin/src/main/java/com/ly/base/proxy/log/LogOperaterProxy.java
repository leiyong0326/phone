 ...
 */
@Component
public class LogOperaterProxy {

	@Autowired
	private LogOperaterConsumerService service;
	/**
	 * 通过主键查询
	 * 
	 * @param pk
	 ...
	 * @return
	 */ 
	public Json insert(HttpServletRequest request,LogOperater data) {
		SysUser user = WebUtils.getLoginUser(request);
	 ...
	 * @return
	 */ 
	public Json insertBatch(HttpServletRequest request,List<LogOperater> list) {
		if(list == null) {return ErrorConfig.getSystemParamErrorJson();};
		SysUser user = WebUtils.getLoginUser(request);
		String checkResult = null;
		for(LogOperater data : list) {
	 ...
	 * @return
	 */ 
	public Json update(HttpServletRequest request,LogOperater data) {
		SysUser user = WebUtils.getLoginUser(request);
		String checkRes = checkData(data,user);
		if (checkRes != null) {
			return new Json().setMsg(checkRes);
	 ...
	 * @return
	 */ 
	public Json findByPage(HttpServletRequest request,LogOperater queryInfo, int pageNum, int pageSize,String orderBy) {
	 ...
	 * @return
	 */ 
	public Json findAll(HttpServletRequest request,LogOperater queryInfo, String orderBy) {
		//queryInfo->conditions
		List<Model> conditions = getConditions(queryInfo,request);
	 ...
	 * @return
	 */ 
	private String checkData(LogOperater data,SysUser user) {
	 ...
	 * @return
	 */ 
	private List<Model> getConditions(LogOperater queryInfo,HttpServletRequest request) {
		if(queryInfo == null){

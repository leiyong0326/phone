package com.ly.base.service.consumerImpl.log;

import com.ly.base.core.model.log.LogOperater;
import com.ly.base.core.provide.log.LogOperaterService;
import com.ly.base.service.consumer.log.LogOperaterConsumerService;
 ...
 */
@Component
public class LogOperaterConsumerServiceImpl implements LogOperaterConsumerService {

	@Autowired
	private LogOperaterService service;
	/**
	 * 通过主键查询
	 * 
	 * @param pk
	 ...
	 */ 
	@Override
	@Logger(model = LogEnum.LogOperater, type = LogOperateEnum.Select, title = "#pk")
	public Json getByPk(Integer pk) {
		Json json = new Json();
		LogOperater result = service.selectByPrimaryKey(pk);
		if (result == null) {
	 ...
	 */ 
	@Override
	@Logger(model = LogEnum.LogOperater, type = LogOperateEnum.Delete, title = "#pk")
	 ...
	 */ 
	@Override
	@Logger(model = LogEnum.LogOperater, type = LogOperateEnum.Delete, title = "#pks?.![#this]")
	 ...
	 */ 
	@Override
	@Logger(model = LogEnum.LogOperater, type = LogOperateEnum.Update, title = "#pk")
	 ...
	 */ 
	@Override
	@Logger(model = LogEnum.LogOperater, type = LogOperateEnum.Update, title = "#pks?.![#this]")
	 ...
	 */ 
	@Override
	@Logger(model = LogEnum.LogOperater, type = LogOperateEnum.Save, title = "#data?.pk")
	public Json insert(LogOperater data) {
		Json json = new Json();
		LogOperater result = service.insertSelective(data);
		if (result == null) {
	 ...
	 */ 
	@Override
	@Logger(model = LogEnum.LogOperater, type = LogOperateEnum.Save, title = "#objs?.!['1']")
	 ...
	 */ 
	@Override
	@Logger(model = LogEnum.LogOperater, type = LogOperateEnum.Update, title = "#data?.pk")
	 ...
	 */ 
	@Override
	@Logger(model = LogEnum.LogOperater, type = LogOperateEnum.Select, title = "")
	public Json findByPage(LogOperater queryInfo, int pageNum, int pageSize,String orderBy) {
		Json json = new Json();
		Page<LogOperater> result = service.findByPage(queryInfo,pageNum,pageSize,orderBy);
		if (result == null) {
			return ErrorConfig.getSystemErrorJson();
		}
	...
	public Json findAll(LogOperater queryInfo, String orderBy) {
		Json json = new Json();
		Page<LogOperater> result = service.findAll(queryInfo,orderBy);
		if (result == null) {
			return ErrorConfig.getSystemErrorJson();
		}
		json.setSuccess(true).setObj(result);
	...
	public Json findByPage(List<Model> conditions, int pageNum, int pageSize,String orderBy) {
		Json json = new Json();
		Page<LogOperater> result = service.findByPage(conditions,pageNum,pageSize,orderBy);
		if (result == null) {
			return ErrorConfig.getSystemErrorJson();
		}
		json.setSuccess(true).setObj(result).setTotal(result.getPages());
	...
	public Json findAll(List<Model> conditions, String orderBy) {
		Json json = new Json();
		Page<LogOperater> result = service.findAll(conditions,orderBy);
		if (result == null) {
			return ErrorConfig.getSystemErrorJson();
		}
		json.setSuccess(true).setObj(result);

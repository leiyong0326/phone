...
function(r) {
    ...
    function($, fmt, w, utils, Temp, Page, datepicker) {
        ...
        function PageRender(options) {
            ...
            this.dataApi = {
                search: 'goodsPhone/findByPage',
                // 查询接口
                update: 'goodsPhone/update',
                // 修改状态
                save: 'goodsPhone/insert',
                // 修改状态
                enable: 'goodsPhone/enable',
                // 修改状态
                disable: 'goodsPhone/disable',
                // 修改状态
                del: 'goodsPhone/deleteByBatch',
                // 删除接口
                searchOne: 'goodsPhone/get'
            };
            // 默认查询参数
            this.searchOptions = {
                pageNum: 1,
            ...
            this.addModal.modal({
                ...
                onConfirm: function() {
                    if(that.eventType!='show'){
                         // 遍历数据
                        ['name', 'seq','typePk','detail'].map(function(i) {
                            that.addOptions[i] = that.addModal.find('[name="' + i + '"]').val();
                        });
                        var typePk = that.addModal.find('[name="typePk"]').val();
                        that.addOptions['typeName'] = that.addModal.find('[name="typePk"]').find('option[value="'+typePk+'"]').text();
		...
		utils.ajax({
		    ...
		    success : function(data){
			 var obj = data.obj;
                         that.addModal.find('[name="typePk"]').createSelect({
                             data: obj,
                             attr: ['pk', 'name'],
                             selected: that.addOptions.typePk
                         });
		    }
		});
            },

define([ '../require.config' ], function(r) {
	...
	function PageRender(options) {
	    ...
	    this.dataApi = {
		search : 'carTestDriveOrder/find',// 查询接口
		del : 'carTestDriveOrder/delete', // 删除接口
		updateStaus: 'carTestDriveOrder/updateStatusByPk'	//修改状态
	    };
	    ...
	    // 按钮组合
	    this.elements = {
		search : $('[data-carTestDriveOrder="query"]'),
		del : $('[data-carTestDriveOrder="del"]')
	    };
	...
	PageRender.prototype = {
		...
		return data.map(function(item) {
			...
			createTime : utils.dateFormat(item.createTime,
				'YYYY-MM-DD hh:mm'),
			testTime : utils.dateFormat(item.testTime,
				'YYYY-MM-DD')
		    };
		});
	    },
